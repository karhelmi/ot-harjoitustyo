...
class ItemListUI:
    def __init__(self, root, act_show_logout_view, act_show_item_view):
        self._root = root
    ...
    def pack(self):
        ...
        self.tree.pack(fill=constants.X)
    ...
    def show_item_list_ui(self): ###
        ...
        #self.frame.grid_rowconfigure(1, weight=1, minsize=1000)

        button_logout = tk.Button(
            master=self.frame, text="Kirjaudu ulos", command=self.handle_logout_button_click)
        button_logout.grid(row=3, column=0, columnspan=2, padx=5, pady=5)

        button_add_item = tk.Button(
            master=self.frame, text="Lisää uusi tarvike", command=self.handle_add_item_button_click)
        button_add_item.grid(row=3, column=3, columnspan=2, padx=5, pady=5)


        self.tree = ttk.Treeview(master=self._root, column=("c1","c2","c3", "c4", "c5", "c6"), show="headings")
        ...
        for row in all_items_table:
            self.tree.insert("", tk.END, values=(row[1], row[2], row[3], row[4], row[5], row[6]))
